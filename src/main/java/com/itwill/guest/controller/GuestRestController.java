...
public class GuestRestController {
	...
	public Map guest_list() throws Exception {
		...
		int status = 1;
		String msg = "성공";
		List<GuestDto> data = new ArrayList<GuestDto>();
		/**************************************************/
	...
	public Map guest_detail(@PathVariable(value = "guest_no") int guest_no) throws Exception {
		...
		GuestDto guest = guestService.guestDetail(guest_no);
		if (guest != null) {
			data.add(guest);
		} else {
			status = 2;
	...
	public Map guest_write_action(@RequestBody GuestDto guestDto) {
		System.out.println(">>guest:" + guestDto);
		Map<String, Object> resultMap = new HashMap<>();
		int status = 1;
		String msg = "성공";
		List<GuestDto> data = new ArrayList<GuestDto>();
		...
		/******************************************/
		try {
			int insert_guest_no = guestService.guestWrite(guestDto);
			GuestDto newGuest = guestService.guestDetail(insert_guest_no);
			status = 1;
			msg = "성공";
			data.add(newGuest);
		} catch (Exception e) {
	...
	public Map guest_modify_action(@PathVariable(name="guest_no") int guest_no ,@RequestBody GuestDto guestDto) {
		...
		/******************************************/
		try {
			guestDto.setGuestNo(guest_no);
			guestDto.setGuestDate(new Date());
			guestService.guestUpdate(guestDto);
			status=1;
			msg="수정성공";
			data.add(guestDto);
		} catch (Exception e) {
